...
void output_file_headers()
{
  ...
  */
  ...
  /* Set up output files (history and solution)  */
    ...
    fp1 = fopen("/Users/xuhui/Desktop/IntroCFD/Cavity/Cavity/history.dat","w");
    fprintf(fp1,"TITLE = \"Cavity Iterative Residual History\"\n");
    fprintf(fp1,"variables=\"Iteration\"\"Time(s)\"\"Res1\"\"Res2\"\"Res3\"\n");

    fp2 = fopen("/Users/xuhui/Desktop/IntroCFD/Cavity/Cavity/cavity.dat","w");
...
void initial(int* ninit, double* rtime, double resinit[neq])
{
  ...
  else
  {
    ...
    {
      fp4 = fopen("/Users/xuhui/Desktop/IntroCFD/Cavity/Cavity/restart.in","r"); /* Note: 'restart.in' must exist! */
      if (fp4==NULL)
      {
...
void write_output(int n, double resinit[neq], double rtime)
{
  ...
  fp3 = fopen("/Users/xuhui/Desktop/IntroCFD/Cavity/Cavity/restart.out","w");
  fprintf(fp3,"%d %e\n", n, rtime);
...
/* !************ADD CODING HERE FOR INTRO CFD STUDENTS************ */
/* !************************************************************** */
    /* Inner node other than the first row/column*/
    for (i=2; i< imax-2; i++){
        for (j=2; j<jmax-2; j++){
            uvel2 = u[i][j][1] * u[i][j][1] + u[i][j][2] * u[i][j][2];
            beta2 = max(uvel2, rkappa * vel2ref);
            lambda_x = half*(abs(u[i][j][1]) + sqrt(u[i][j][1]*u[i][j][1] + four*beta2));
            lambda_y = half*(abs(u[i][j][2]) + sqrt(u[i][j][2]*u[i][j][2] + four*beta2));
            d4pdx4 = (u[i+2][j][0]*one - u[i+1][j][0]*four + u[i][j][0]*six - u[i-1][j][0]*four + u[i-2][j][0]*one)/pow(dx,4);
            d4pdy4 = (u[i][j+2][0]*one - u[i][j+1][0]*four + u[i][j][0]*six - u[i][j-1][0]*four + u[i][j-1][0]*one)/pow(dy,4);
            artviscx[i][j] = - lambda_x * Cx * pow(dx,3)*d4pdx4/beta2;
            artviscy[i][j] = - lambda_y * Cy * pow(dy,3)*d4pdy4/beta2;
        }
    }
...
/* !************************************************************** */
    for (j=1; j< jmax-1; j++){
        for (i=1; i<imax-1; i++){
            //time-direvative preconditioning term
            uvel2 = uold[i][j][1] * uold[i][j][1] + uold[i][j][2] * uold[i][j][2];
            beta2 = max(uvel2, rkappa * vel2ref);
            // Derivatives
            dudx = (uold[i+1][j][1]-u[i-1][j][1])/(two*dx);
            dvdy = (uold[i][j+1][2]-u[i][j-1][2])/(two*dy);
            dudy = (uold[i][j+1][1]-u[i][j-1][1])/(two*dy);
            dpdx = (uold[i+1][j][0]-u[i-1][j][0])/(two*dx);
            d2udx2 = (uold[i+1][j][1]-two*uold[i][j][1]+u[i-1][j][1])/(pow(dx,2));
            d2udy2 = (uold[i][j+1][1]-two*uold[i][j][1]+u[i][j-1][1])/(pow(dy,2));
            dvdx = (uold[i+1][j][2]-u[i-1][j][2])/(two*dx);
            dpdy = (uold[i][j+1][0]-u[i][j-1][0])/(two*dy);
            d2vdx2 = (uold[i+1][j][2]-two*uold[i][j][2]+u[i-1][j][2])/(pow(dx,2));
            d2vdy2 = (uold[i][j+1][2]-two*uold[i][j][2]+u[i][j-1][2])/(pow(dy,2));
            // update u
            u[i][j][0] = uold[i][j][0] - beta2 * dt[i][j] * (rho*dudx + rho*dvdy - artviscx[i][j] - artviscy[i][j] - s[i][j][0]);
            u[i][j][1] = uold[i][j][1] - dt[i][j] * rhoinv * (rho * uold[i][j][1] * dudx + rho * uold[i][j][2] * dudy + dpdx - rmu*(d2udx2 + d2udy2) - s[i][j][1]);
            u[i][j][2] = uold[i][j][2]- dt[i][j] * rhoinv * (rho * uold[i][j][1] * dvdx + rho * uold[i][j][2] * dvdy + dpdy - rmu*(d2vdx2 + d2vdy2) - s[i][j][2]);
        }
    }
...
/* !************************************************************** */
    for (i=imax-2; i>0; i--){
        for (j=jmax-2; j>0; j--){
            ...
            beta2 = max(uvel2, rkappa * vel2ref);
            // Derivatives
            dudx = (u[i+1][j][1]-uold[i-1][j][1])/(two*dx);
            dvdy = (u[i][j+1][2]-uold[i][j-1][2])/(two*dy);
            dudy = (u[i][j+1][1]-uold[i][j-1][1])/(two*dy);
            dpdx = (u[i+1][j][0]-uold[i-1][j][0])/(two*dx);
            d2udx2 = (u[i+1][j][1]-two*uold[i][j][1]+uold[i-1][j][1])/(pow(dx,2));
            d2udy2 = (u[i][j+1][1]-two*uold[i][j][1]+uold[i][j-1][1])/(pow(dy,2));
            dvdx = (u[i+1][j][2]-uold[i-1][j][2])/(two*dx);
            dpdy = (u[i][j+1][0]-uold[i][j-1][0])/(two*dy);
            d2vdx2 = (u[i+1][j][2]-two*uold[i][j][2]+uold[i-1][j][2])/(pow(dx,2));
            d2vdy2 = (u[i][j+1][2]-two*uold[i][j][2]+uold[i][j-1][2])/(pow(dy,2));
            // update u
            u[i][j][0] = uold[i][j][0] - beta2 * dt[i][j] * (rho*dudx + rho*dvdy - artviscx[i][j] - artviscy[i][j] - s[i][j][0]);
            u[i][j][1] = uold[i][j][1] - dt[i][j] * rhoinv * (rho * uold[i][j][1] * dudx + rho * uold[i][j][2] * dudy + dpdx - rmu*(d2udx2 + d2udy2) - s[i][j][1]);
            u[i][j][2] = uold[i][j][2]- dt[i][j] * rhoinv * (rho * uold[i][j][1] * dvdx + rho * uold[i][j][2] * dvdy + dpdy - rmu*(d2vdx2 + d2vdy2) - s[i][j][2]);
...
/* !************************************************************** */
    ...
    for (i=0; i<imax; i++){
        for (j=0; j<jmax; j++){
            for(k=0; k< neq; k++){
                res[k] = res[k] + pow((u[i][j][k] - uold[i][j][k])/(dt[i][j]+fsmall),2);
            }
        }
    }
    for(k=0; k< neq; k++){
      ...
      for (k = 0; k < neq; k++){
          for(i=0;i<imax;i++){
              for(j = 0; j<jmax; j++){
                  x = (xmax - xmin)*(double)(i)/(double)(imax - 1);
                  y = (ymax - ymin)*(double)(j)/(double)(jmax - 1);
                  DE = u[i][j][k]-umms(x,y,k);
                  rL1norm[k] = rL1norm[k] + abs(DE)/(imax*jmax);
                  rL2norm[k] = rL2norm[k] + pow(DE,2)/(imax*jmax);
                  rLinfnorm[k] = max(rLinfnorm[k], abs(DE));
              }
          }
          rL2norm[k] = sqrt(rL2norm[k]);
      }

      fp5 = fopen("/Users/xuhui/Desktop/IntroCFD/Cavity/Cavity/norm_DE.dat","w");
      fprintf(fp5,"TITLE = \"Cavity Discretization Error Norms\"\n");
      fprintf(fp5,"variables=\"Variable\"\"L1\"\"L2\"\"Linf\"\n");
      for(k=0;k<neq;k++){
